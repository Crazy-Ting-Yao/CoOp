...
class Retinal(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        if num_shots <= 1:
            train_name = "andyqmongo/IVL_CLS_retinal_1_shot"
        else:
            train_name = "andyqmongo/IVL_CLS_retinal_8_shot"

        print(f"Loading Retinal from HuggingFace (train={train_name})...")
        train_raw = _load_hf_dataset(train_name, "train")
        test_raw = _load_hf_dataset("andyqmongo/retinal_eval_standard", "test")
        ...
        print(
            f"Retinal: {len(classname_to_label)} classes, "
