...
class WM811K(DatasetBase):
    """WM811K wafer map classification dataset from HuggingFace."""
    ...
    def __init__(self, cfg):
        ...
        if num_shots <= 1:
            train_name = "andyqmongo/IVL_CLS_WM811k_1_shot"
        else:
            train_name = "andyqmongo/IVL_CLS_WM811k_8_shot"

        print(f"Loading WM811K from HuggingFace (train={train_name})...")
        train_raw = _load_hf_dataset(train_name, "train")
        test_raw = _load_hf_dataset("andyqmongo/WM811k_eval_standard", "test")
        ...
        print(
            f"WM811K: {len(classname_to_label)} classes, "
    ...
    def _get_classname(item):
        for key in ("primary_type", "label", "class", "category"):
