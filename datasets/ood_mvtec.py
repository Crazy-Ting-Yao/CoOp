...
class OOD_MVTec(DatasetBase):
    """MVTec AD OOD dataset from HuggingFace.

    Dataset: andyqmongo/IVL_OOD_MVTec_{object} (config: 1_shot)
    Pass object name via DATASET.SOURCE_DOMAINS, e.g. ['bottle']
    """

    dataset_dir = "ood_mvtec"

    def __init__(self, cfg):
        root = os.path.abspath(os.path.expanduser(cfg.DATASET.ROOT))

        obj = cfg.DATASET.SOURCE_DOMAINS[0] if cfg.DATASET.SOURCE_DOMAINS else "bottle"
        assert obj in MVTEC_OBJECTS, f"Unknown MVTec object: {obj}. Choose from: {MVTEC_OBJECTS}"

        self.dataset_dir = osp.join(root, self.dataset_dir, obj)
        self.image_dir = osp.join(self.dataset_dir, "images")
        mkdir_if_missing(self.image_dir)

        hf_name = f"andyqmongo/IVL_OOD_MVTec_{obj}"

        print(f"Loading OOD_MVTec ({obj}) from HuggingFace ({hf_name})...")
        train_raw = _load_hf_dataset(hf_name, split="1_shot")
        ...
        print(
            f"OOD_MVTec ({obj}): {len(classname_to_label)} classes, "
